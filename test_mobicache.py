...
from kmanga_mobi.cache import DB
from kmanga_mobi.cache import IssueCache
from kmanga_mobi.cache import LockFile
from kmanga_mobi.cache import MobiCache
...
class TestLockFile(unittest.TestCase):
    LOCK = 'fixtures/cache/lock.lck'
    FNAME = 'fixtures/cache/test.txt'
...
class TestDB(unittest.TestCase):
    DBNAME = 'fixtures/cache/test.db'
    DBNAMELCK = 'fixtures/cache/test.db.lck'
...
class TestIssueCache(unittest.TestCase):
    ...
    def setUp(self):
        self.cache = IssueCache('fixtures/tmp', 'fixtures/images')

    def tearDown(self):
        shutil.rmtree('fixtures/tmp')
    ...
    def test_is_valid(self):
        ...
        open('fixtures/images/missing-image.jpg', 'a').close()
        self.cache['url2'] = [{'images': [{'path': 'missing-image.jpg'}]}]
        # Remove temporal image
        os.unlink('fixtures/images/missing-image.jpg')
...
class TestMobiCache(unittest.TestCase):
    ...
    def setUp(self):
        self.cache = MobiCache('fixtures/tmp')

    def tearDown(self):
        shutil.rmtree('fixtures/tmp')

    def test_cache(self):
        self.cache['url1'] = ['fixtures/cache/mobi1.mobi']
        self.cache['url2'] = [
            'fixtures/cache/mobi2.1.mobi',
            'fixtures/cache/mobi2.2.mobi'
        ]
        self.cache['url3'] = ['fixtures/cache/mobi3.mobi']
        self.assertTrue(len(self.cache) == 3)

        md5 = hashlib.md5(b'url1').hexdigest()
        self.assertEqual(
            self.cache['url1'][0],
            [('mobi1.mobi', 'fixtures/tmp/data/%s-00' % md5)]
        )
    ...
    def test_clean(self):
        self.cache['url1'] = ['fixtures/cache/mobi1.mobi']
        self.cache['url2'] = ['fixtures/cache/mobi2.1.mobi']
        time.sleep(2)
        self.assertTrue(len(self.cache) == 2)
        self.cache['url3'] = ['fixtures/cache/mobi3.mobi']
        self.assertTrue(len(self.cache) == 3)
        ...
        time.sleep(2)
        self.assertTrue(len(self.cache) == 2)
        self.cache['url2'] = ['fixtures/cache/mobi2.2.mobi']
        self.cache['url3'] = ['fixtures/cache/mobi3.mobi']
        self.assertTrue(len(self.cache) == 3)
        self.cache.clean(ttl=1)
        self.assertTrue(len(self.cache) == 2)
    ...
    def test_free(self):
        ...
        self.cache.nclean = 2
        self.cache['url1'] = ['fixtures/cache/mobi1.mobi']
        self.cache['url2.1'] = ['fixtures/cache/mobi2.1.mobi']
        self.cache.free()
        self.assertTrue(len(self.cache) == 2)

        self.cache['url2.2'] = ['fixtures/cache/mobi2.2.mobi']
        self.cache['url3'] = ['fixtures/cache/mobi3.mobi']
        self.cache.free()
        self.assertTrue(len(self.cache) == 2)
        self.assertTrue('url1' not in self.cache)
